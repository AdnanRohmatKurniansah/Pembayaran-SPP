            ...
            <div class="col-md-10">
                <div class="card">
                    <div class="card-content">
                        <div class="card-body">
                            <form class="form form-vertical" action="/dashboard/tagihan" method="POST">
                                @csrf
                                <div class="form-body">
                                    <div class="row">
                                        <div class="col-12">
                                            <div class="form-group">
                                                <label for="id_spp">Siswa</label>
                                                <select class="form-select @error('id_siswa') is-invalid @enderror" value="{{ old('id_siswa') }}"  name="id_siswa">
                                                    @foreach ($siswas as $siswa)
                                                      @if(old('id_siswa') == $siswa->id)
                                                        <option value="{{ $siswa->id }}" selected>{{ $siswa->nis }} - {{ $siswa->nama }} - {{ $siswa->nisn }} - {{ $siswa->kelas->nama_kelas }}</option>
                                                      @else
                                        ...
                                        <div class="col-12">
                                            <div class="form-group">
                                                ...
                                                <select class="form-select @error('id_spp') is-invalid @enderror" value="{{ old('id_spp') }}"  name="id_spp">
                                                    @foreach ($spps as $spp)
